...
<html lang="en">
    ...
    <body>
        <?php
            ...
            require "Producao/OrdemParametrizacao.php";

            $op = trim($_GET["op"] ?? $_POST["op"] ?? NULL);
            $idReceita = trim($_GET["idReceita"] ?? $_POST["idReceita"] ?? NULL);
            ...
            if($_POST){
                ...
                $O = new OrdemParametrizacao ($idReceita, $idItem, $qtd, $idUnidadeMedida);

                $resultado = $O->cadastrarIngrediente();
            ...
            if($op == "excluir"){
                ...
                $Itens = new OrdemParametrizacao($idReceita, $idItem, NULL, NULL, NULL);
                $resultado = $Itens->excluirIngrediente();
            ...
            $sql = "select i.idItem, i.nome, itens.quantidade, un.nome uniMedida from ordemparametrizacao itens
            inner join item i on (itens.idItem = i.idItem)
            inner join unidademedida un on (itens.idUnidadeMedida = un.idUnidadeMedida)
...
<script>
    function Excluir(id, id2){
        var idReceita = <?=$idReceita?>;
        location.href = "excluir/itensReceita/" + id + "/" + id2;
    }
</script>
